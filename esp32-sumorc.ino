...
// Constructor: Motor(int In1pin, int In2pin, int PWMpin, int offset, int STBYpin, int freq, int resolution, int channel_pin);
Motor motor1(AIN1, AIN2, PWMA, 10, STBY, 2000, 8, 0);  // Motor 1 (Motor A): usa pines AIN1, AIN2, PWMA, y el pin STBY compartido
Motor motor2(BIN1, BIN2, PWMB, 11, STBY, 2000, 8, 1);  // Motor 2 (Motor B): usa pines BIN1, BIN2, PWMB, y el pin STBY compartido
...
void processGamepad(ControllerPtr ctl) {
    ...
    int speedR = ctl->axisRX();  // Eje R: izquierda/derecha (valor entre -512 y 511)
    ...
    // Si el joystick de movimiento (izquierdo, eje Y) está fuera de la zona muerta
    if (abs(motorSpeedL) > deadZone) {
        // Movimiento adelante/atrás
        if (motorSpeedL < 0) { 
             // Joystick hacia arriba (speedL negativo), vamos a mapear esto a "adelante" con velocidad positiva
            forward(motor1, motor2, abs(motorSpeedL)); // Usamos valor absoluto para la velocidad
            ...
            back(motor1, motor2, abs(motorSpeedL)); // Usamos valor absoluto para la velocidad
        ...
        if (abs(motorSpeedR) <= deadZone) {
             brake(motor1, motor2);
             Serial.println("En zona muerta (adelante/atrás) y (izquierda/derecha) - frenando");
        } else {
             // Si solo el eje de movimiento está en zona muerta, pero el eje de giro no,
             // no frenamos, permitimos el giro en el sitio.
    ...
    if (abs(motorSpeedR) > deadZone) {
        ...
        if (motorSpeedR > 0) { // Joystick hacia la derecha
            right(motor1, motor2, abs(motorSpeedR)); // Usamos valor absoluto para la velocidad de giro
            Serial.printf("Girando hacia la derecha a velocidad: %d\n", abs(motorSpeedR));
        } else { // Joystick hacia la izquierda
            left(motor1, motor2, abs(motorSpeedR)); // Usamos valor absoluto para la velocidad de giro
            Serial.printf("Girando hacia la izquierda a velocidad: %d\n", abs(motorSpeedR));
        }
    } else {
         // Si el eje de giro está en zona muerta y también lo estaba el eje de movimiento, ya frenamos arriba.
         // Si el eje de movimiento no estaba en zona muerta, seguimos moviéndonos adelante/atrás sin girar.
         if (abs(motorSpeedL) > deadZone) {
             Serial.println("Eje izquierda/derecha en zona muerta, pero moviéndose adelante/atrás.");
         }
    }
...
void loop() {
    ...
    // si necesitas una respuesta más rápida del robot. Un valor entre 10-50ms suele ser suficiente.
